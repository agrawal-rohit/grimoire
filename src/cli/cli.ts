...
class CLI {
	...
	/**
	 * Prompt for a multi-selection input.
	 * @param message - The prompt message to display.
	 * @param opts - Options for the multiselect prompt (excluding message).
	 * @param defaultValues - Optional initial values to preselect.
	 * @returns The selected values.
	 */
	async multiselectInput<Value extends string | number = string>(
		message: string,
		opts: Omit<MultiSelectOptions<Value>, "message"> = { options: [] },
		defaultValues: Value[] | undefined = undefined,
	): Promise<Value[]> {
		if (defaultValues && defaultValues.length > 0)
			opts.initialValues = defaultValues;
