...
describe('cli/logger', () => {
  ...
  describe('intro', () => {
    test('should call animatedIntro with the provided message', async () => {
      const testMessage = 'Welcome to Grimoire!'
  ...
  describe('error', () => {
    test('should log error message with proper formatting', () => {
      ...
      expect(() => loggerInstance.error(errorMessage)).toThrow('process.exit called with code 1')
    ...
    test('should exit with code 1', () => {
      ...
      expect(() => loggerInstance.error('Test error')).toThrow('process.exit called with code 1')
  ...
  describe('end', () => {
    test('should log end message with proper formatting', () => {
      ...
      expect(() => loggerInstance.end(endMessage)).toThrow('process.exit called with code 0')

      expect(consoleLogSpy).toHaveBeenCalled()
      expect(consoleErrorSpy).toHaveBeenCalled()
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining(endMessage))
    })

    test('should exit with code 0', () => {
      const loggerInstance = new Logger()

      expect(() => loggerInstance.end('Test end')).toThrow('process.exit called with code 0')
